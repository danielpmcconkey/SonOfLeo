...
- Source code carries `// REQ-AC-1.5` annotations at each site that enforces the
  requirement — every site, not just the first. That is how a future developer knows what
  bears load when the rule changes.
- Test names begin with the requirement IDs they verify.
- The requirement→code and requirement→test maps are generated by tooling
