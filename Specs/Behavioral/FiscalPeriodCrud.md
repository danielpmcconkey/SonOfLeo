...
- **REQ-FP-1.2** Fiscal period key must match the format `YYYY-MM` where YYYY is a four-digit year and MM is a two-digit month (01–12). The regex pattern is `^\d{4}-\d{2}$` with MM constrained to 01–12.
