...
- **FT-DAL-1.8** The "SonOfLeo" connection string will not print the database password in the external configuration file
...
- **FT-DAL-3.2** The DAL modules must build abstraction layers such that callers of DAL modules need not require any reference to PostgreSQL (preserving the ability to shift RDBMS architecture without upending the entire application).
  - **FT-DAL-3.2.1** An exception to FT-DAL-3.2 is that client modules can pass non-Ansi-generic SQL strings to the DAL if needed.
  - **FT-DAL-3.2.2** An exception to FT-DAL-3.2 is that customer-facing applications (e.g.: SonOfLeoCli) will need to create RDBMS-specific connection strings in their external configurations.
