...
- **REQ-JE-4.11** When the voided journal entry was posted from a staged entry, the void must, in the same transaction: transition that staged entry from `'Posted'` to `'Reviewed'` (REQ-STG-4.7), and clear the staged entry's and its lines' records of the journal entry and journal entry lines they produced (REQ-STG-9.10).
  - *Why:* Correcting an imported transaction is void-and-repost. The staged entry goes back to review so the operator can fix it and post it again with its provenance intact. (2026-09-26)
- **REQ-JE-4.12** When a Payment's transaction pointer references a line of the voided journal entry, the void must, in the same transaction: clear the Payment's journal entry line ID when the Payment also references a staged line, so the pointer resolves to Staged again; and re-derive the affected Invoice's and Instance's states (REQ-CF-9.10). When such a Payment references no staged line, the void is rejected with a typed error naming the Payment.
  - *Why:* Without this, a voided entry leaves an Invoice reading 'FullyPaid' and 'PostedToLedger' on the strength of cash the ledger no longer records, and projection and bills-to-chase go quietly wrong. A Payment with no staged origin has nowhere to fall back to; the operator removes or re-points it deliberately before voiding. (2026-09-26)
