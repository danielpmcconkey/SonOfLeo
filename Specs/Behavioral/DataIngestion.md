...
- **REQ-STG-3.12** On successful ingestion, the file is moved to the processed directory, its name prefixed with the ingestion timestamp (`yyyy-MM-dd.HHmmss.fff-`). The move happens only after the staged data has been committed. A failed ingestion — including a failure to commit — leaves the file where it was. If the data commits but the move then fails, the operation reports a distinct error stating that the entries were staged and the file must be moved by hand; re-ingesting it would stage the same entries again, which dedup then flags (REQ-STG-7.2).
...
- **REQ-STG-4.8** Two transitions in the table are reserved to one operation each: `→ 'Posted'` is made only by batch post (§9), and `'Posted' → 'Reviewed'` only by the void reversal (REQ-STG-4.7). Every other operation, including the manual update (§6), must reject both, even though they appear in the table.
  - *Why:* An operator moving an entry to `'Posted'` by hand records a posting that never happened; moving it out of `'Posted'` without voiding allows the same transaction to post twice. (2026-09-26)
...
- **REQ-STG-6.4** The system must provide a means for the operator to add lines to a staged entry, and to remove lines from it subject to REQ-STG-6.5, in the same operation as any other manual update (REQ-STG-6.2). Validation applies to the entry as it stands after the whole operation: it must satisfy every staged-entry requirement in §2, in particular at least two lines (REQ-STG-2.9) and balance (REQ-STG-2.17). An operation whose result fails validation changes nothing. A split is expressed as reducing an existing line's amount and adding new lines; it does not require removing the original.
  - *Why:* One FI line often carries more than one economic purpose. A tenant payment covers rent and a utility share that post to different revenue accounts; a mortgage payment covers principal, interest and escrow. The split depends on data the parser may not have when it runs (the tenant's invoice can postdate the parse), so the operator must be able to split in review. Validating only the final state is what makes a split possible at all: reducing one line and adding another passes through an unbalanced intermediate. Splitting by edit-and-add rather than remove-and-replace keeps the classification history attached to the original line (REQ-STG-6.5). (2026-09-26, revised same day)
- **REQ-STG-6.5** A staged line cannot be removed when it is linked to a payment agreement (CashFlow §12), referenced by a Payment, or recorded in any classification run (REQ-STG-5.10). A line cannot have its amount, line type or account changed while it is linked or referenced by a Payment. Either attempt fails with a typed error naming the line and the reason; the link or Payment must be removed first. Only lines no classification run has evaluated — in practice, lines the operator added — can be removed.
  - *Why:* Removing the line would orphan the link or Payment that points at it, and classification-run records are permanent (REQ-CR-8.4). Changing a linked or paid line's amount silently changes the Payment's derived amount and the Invoice's paid total without re-deriving its state; changing its account or line type can invalidate the leg selection that produced the link. (2026-09-26, revised same day)
- **REQ-STG-6.6** A staged entry whose status is `'Posted'` cannot be modified by the manual update in any way — no field edits on the entry or its lines, no line additions or removals. (Revised 2026-09-26)
  - *Why:* A posted entry's lines are paired to its journal entry's lines (REQ-STG-9.10). Editing them desynchronises staging from the ledger. Corrections go through void and repost (REQ-STG-4.7). (2026-09-26)
- **REQ-STG-6.7** A staged entry with any line referenced by a Payment cannot transition to `'Duplicate'` or `'Ignored'`, by any operation. The attempt fails with a typed error (manual update) or is reported without flagging (dedup, transfer pairing).
  - *Why:* The Payment would be left proving a cash movement that the system has since decided did not happen. The operator removes the Payment first if the entry really is a duplicate. (2026-09-26)
...
- **REQ-STG-7.7** Two staged entries are counterparts when all of the following hold: (a) they come from different sources; (b) every line on both entries has an assigned account; (c) the two entries would produce the same journal entry lines — compared as multisets of (account, line type, amount), so line order and repeated identical lines are handled; (d) the absolute difference between their entry dates is no more than a caller-supplied number of days (zero or more).
...
- **REQ-STG-7.9** Candidates and their counterparts form groups: the connected components of the counterpart relation. (The date window is not transitive, so a group can contain entries further apart than the window.) A group of exactly two entries is resolved when exactly one of them can be flagged. The entry flagged is chosen by the first rule that decides it: (1) an entry whose counterpart is `'Posted'` is flagged; (2) an entry whose counterpart is `'Reviewed'` is flagged; (3) the entry with the later entry date is flagged. An entry is never flagged when it is `'Posted'` or `'Reviewed'` or when REQ-STG-6.7 forbids it. If no rule decides, or the decided entry cannot be flagged, the pair is reported and neither is flagged. The flagged entry transitions to `'Duplicate'` with change mechanism `'Deduplicator'`.
  - *Why:* `'Posted'` and `'Reviewed'` entries are never flagged: posting is final until voided, and the operator outranks the deduplicator (REQ-STG-7.2). Otherwise the earlier date survives because it is when the money left, which is the date the journal entry should carry. Two entries on the same date are reported rather than resolved by ingestion order or ID — that would be code breaking a tie. (2026-09-26, revised same day)
- **REQ-STG-7.10** A group of more than two entries is never resolved. No entry in it is flagged, and the group is reported for the operator.
...
- **REQ-STG-7.11** The pairing operation returns every resolved pair (survivor and flagged entry) and every unresolved group, with the reason each was not resolved.
