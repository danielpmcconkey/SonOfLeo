...
//         let payload = accountInput |> toJson<AccountCreateInput> |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
//         let code, a, e = runCli args payload
//         match code with
//         | 0 ->
//             let accountReturn:AccountReturn = fromJson<AccountReturn> a |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
//             let cleanUpId = accountReturn.code |> LookupCache.accountCodeToId.fetch |> Result.defaultWith (fun e -> failwith (AppError.toMessage e)) |> AccountId.fromGuid
//             cleanUpAccountId (Some cleanUpId) |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
//         | _ ->
