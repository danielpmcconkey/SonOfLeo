...
type JournalEntryVoidingTests(fixture: TestDataFixture) =
    ...
    let commentText = "Voiding for test" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
    ...
    member _.``REQ-JE-4.3 voidJournalEntryOrchestration sets voided_at on the entry`` () =
        ...
        try
            let railroad = result {
                let! _, jeId = createTestJournalEntryFromPrimitives "JE create for void 4.3" None today
    ...
    member _.``REQ-JE-4.4 voidJournalEntryOrchestration attaches a reason comment to the voided entry`` () =
        ...
        try
            let railroad = result {
                let! je, jeId = createTestJournalEntryFromPrimitives "JE create for void 4.4" None today
                                    [ (fixture.Data.entertainment5650Id, 86.04M, "Debit", None)
                                      (fixture.Data.creditCard2220Id, 86.04M, "Credit", None) ]
                                      [] [] envelopeCreate
                idToCleanUp <- Some jeId
                ...
                Assert.Equal(commentText |> CommentText.value, comment |> JournalEntryComment.commentText |> CommentText.value)
                return () }
            match railroad with
            | Ok _ -> ()
            | Error e -> Assert.Fail (AppError.toMessage e)
    ...
    member _.``REQ-JE-4.5 voidJournalEntryOrchestration rejects void when fiscal period is closed`` () =
        ...
        try
            let railroad = result {
                ...
                let! _, jeId = createTestJournalEntryFromPrimitives "JE create for void 4.5" None sevenMonthsAgo
                                    [ (fixture.Data.entertainment5650Id, 86.04M, "Debit", None)
                                      (fixture.Data.creditCard2220Id, 86.04M, "Credit", None) ]
                                      [] [] envelopeCreate
                idToCleanUp <- Some jeId
