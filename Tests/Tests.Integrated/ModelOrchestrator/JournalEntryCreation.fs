...
type JournalEntryCreationTests(fixture: TestDataFixture) =
    ...
    member _.``REQ-JE-2.13 REQ-JE-2.11 orchestrateCreation posts a valid journal entry and returns it`` () =
        ...
        try
            ...
            | Ok je ->
                idToCleanUp <- Some (je |> header |> JournalEntryHeader.journalEntryHeaderId)
                let h = je |> header
    ...
    member _.``REQ-JE-2.1 orchestrateCreation generates a unique UUID for the header`` () =
        ...
        try
            ...
            | Ok je ->
                idToCleanUp <- Some (je |> header |> JournalEntryHeader.journalEntryHeaderId)
                Assert.NotEqual(Guid.Empty, je |> header |> JournalEntryHeader.journalEntryHeaderId)
            | Error e -> Assert.Fail e
        finally
            match cleanUpJournalEntryId idToCleanUp with
            | Ok () -> ()
    ...
    member _.``REQ-JE-1.48 orchestrateCreation accepts duplicate source_fi/reference pairs across entries`` () =
        ...
        try
            ...
            create1 |> Result.iter (fun je ->
                idToCleanUp_1 <- Some (je |> header |> JournalEntryHeader.journalEntryHeaderId))
            ...
            create2 |> Result.iter (fun je ->
                idToCleanUp_2 <- Some (je |> header |> JournalEntryHeader.journalEntryHeaderId))
    ...
    member _.``REQ-SYS-5.1 posted entry round-trips through persistence with all fields intact`` () =
        ...
        try
            ...
            | Error e -> Assert.Fail e
            | Ok created ->
                let jeId = created |> header |> JournalEntryHeader.journalEntryHeaderId
                ...
                | Ok fetched ->
                    ...
                    Assert.Equal(ch |> JournalEntryHeader.journalEntryHeaderId, fh |> JournalEntryHeader.journalEntryHeaderId)
                    Assert.Equal(
