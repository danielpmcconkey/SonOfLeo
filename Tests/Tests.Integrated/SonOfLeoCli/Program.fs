...
type ProgramTests(fixture: TestDataFixture) =
    ...
    member _.``REQ-NGUI-1.3, REQ-NGUI-3.6 System responds with a success code when succeeding`` () =
        ...
        let payload = { activeOnly = true } |> toJson<AccountFetchAllInput> |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
    ...
    member _.``REQ-NGUI-1.3.1, REQ-NGUI-3.7 The stderr will comprise the error message`` () =
        ...
        let payload = { code = code } |> toJson<AccountFetchByCodeInput> |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
