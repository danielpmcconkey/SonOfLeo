...
type JournalEntryCommentTests(fixture: TestDataFixture) =
    ...
    member _.``REQ-JE-5.1 constructNewAndSaveToDb attaches a comment to a journal entry`` () =
        ...
        let commentText = "Test comment text" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
           let result = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                            fixture.Data.basicJeId None commentText envelope (Some transaction)
           match result with
           | Ok c ->
    ...
    member _.``REQ-JE-5.1 constructNewAndSaveToDb attaches a comment with a secondary JE link`` () =
        let envelope = AuditEnvelope.create JournalEntryAddComment
        let transaction =  createDbTransaction() |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        let commentText = "Comment with secondary link" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
           let result = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                            fixture.Data.basicJeId (Some fixture.Data.jeWithRefId)
                            commentText envelope (Some transaction)
           match result with
           | Ok c ->
              Assert.Equal(fixture.Data.basicJeId, c |> JournalEntryComment.primaryJournalEntryId)
    ...
    member _.``REQ-JE-5.2 constructNewAndSaveToDb generates UUID and sets timestamps`` () =
        ...
        try
            ...
            match result with
            | Ok c ->
               Assert.NotEqual(Guid.Empty, c |> JournalEntryComment.journalEntryCommentId |> JournalEntryCommentId.value)
    ...
    member _.``REQ-JE-1.52 constructNewAndSaveToDb accepts null secondary JE ID`` () =
        let envelope = AuditEnvelope.create JournalEntryAddComment
        let transaction =  createDbTransaction() |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        let commentText = "Null secondary" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
            let result = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                            fixture.Data.basicJeId None commentText envelope (Some transaction)
            match result with
    ...
    member _.``REQ-JE-5.5 constructNewAndSaveToDb allows comment on a voided entry`` () =
        let envelope = AuditEnvelope.create JournalEntryAddComment
        let transaction =  createDbTransaction() |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        let commentText = "Comment on voided" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
            let result = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                            fixture.Data.voidedJeId None commentText envelope (Some transaction)
            match result with
    ...
    member _.``REQ-JE-5.5 constructNewAndSaveToDb allows comment when fiscal period is closed`` () =
        let envelope = AuditEnvelope.create JournalEntryAddComment
        let transaction =  createDbTransaction() |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        let commentText = "Comment on closed period entry" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
            let result = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                            fixture.Data.jeInClosedPeriodId None commentText envelope (Some transaction)
            match result with
    ...
    member _.``REQ-JE-5.3 updateComment amends the comment text`` () =
        ...
        try
            let railroad = result {
                ...
                let! updatedComment =
                   JournalEntryCommentOrchestration.updateComment envelope fixture.Data.fixtureCommentId
    ...
    member _.``REQ-SYS-5.1 comment round-trips through persistence with all fields intact`` () =
        let envelope = AuditEnvelope.create JournalEntryAddComment
        let transaction =  createDbTransaction() |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        let commentText = "Round-trip comment" |> CommentText.create |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        try
            let createResult = JournalEntryCommentOrchestration.constructNewAndSaveToDb
                                   fixture.Data.basicJeId (Some fixture.Data.jeWithRefId)
                                   commentText envelope (Some transaction)
