...
type FiscalPeriodTests(fixture: TestDataFixture) =
    ...
    member _.``REQ-FP-2.1 creating a fiscal period must generate a UUID`` () =
        ...
        try
            let railroad = result {
                let! fp = FiscalPeriodCreation.constructNewAndSaveToDb genericFiscalPeriodKey genericAuditEnvelope (Some transaction)
    ...
    member _.``REQ-FP-1.3 REQ-FP-2.2 Period Key must be unique`` () =
        ...
        try
            let railroad = result {
                let! existingPeriod = FiscalPeriod.fetchById (Some transaction) (fixture.Data.openFiscalPeriodIds |> List.head)
                let existingKey = FiscalPeriod.periodKey existingPeriod
                let duplicateResult = FiscalPeriodCreation.constructNewAndSaveToDb existingKey genericAuditEnvelope (Some transaction)
                do! match duplicateResult with
    ...
    member _.``REQ-FP-2.4 REQ-FP-2.5 insertNewToDb happy path`` () =
        let expectedKey = "2050-10" |> FiscalPeriodKey.fromString |> Result.defaultWith (fun e -> failwith (AppError.toMessage e))
        ...
        try
            let railroad = result {
                let! fp = FiscalPeriodCreation.constructNewAndSaveToDb expectedKey genericAuditEnvelope (Some transaction)
