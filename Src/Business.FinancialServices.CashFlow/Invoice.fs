...
let applyFieldUpdates (fieldUpdates: InvoiceFieldUpdates) (invoice: Invoice) : Invoice =
    ...
    { invoice with
        ...
        invoiceDate = fieldUpdates.invoiceDateUpdate |> valueOrCurrent invoice.invoiceDate
        dueDate = fieldUpdates.dueDateUpdate |> valueOrCurrent invoice.dueDate
        amount = fieldUpdates.amountUpdate |> valueOrCurrent invoice.amount
        invoiceLifeCycleState = invoiceLifeCycleState
        memo = fieldUpdates.memoUpdate |> valueOrCurrent invoice.memo }
...
let fetchByInstanceIdList
    ...
    let namesAndParameters =
        ...
        |> List.map (fun (ordinal, id) ->
            let name = $"@instanceId{ordinal}"
            name, { name = name; value = UniqueId(id |> InstanceId.value) })
    let names = namesAndParameters |> List.map fst |> String.concat ", "
    let parameters = namesAndParameters |> List.map snd
