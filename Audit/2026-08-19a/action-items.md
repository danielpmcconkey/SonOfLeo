...
| 18 | IDIOM-JE-3 | Change test citation from JE-4.9 to SYS-6.1. (See #36 for follow-on investigation.) | Dan/BD | blocked on Dan — REQ-SYS-6.1 is waived and carries no test citations; re-citing would make it the first waived-and-tested REQ |
...
| 23 | RPT-SORT-1 | Fix the test to verify depth-first tree order | Dan/BD | done — same fix as #6 |
| 24 | SYS-5.1-PARTIAL | Have BD add equality assertions on the other fields | Dan/BD | done — all ten properties asserted; subtype, parent and external reference populated non-None |
...
| 36 | IDIOM-JE-3a | Move REQ-JE-4.9 UpdateExternalReference no-op test from interface bridge to model orchestrator tests. Rewrite to flow like the comment no-op test (`REQ-JE-4.9 updateComment rejects no-op when both fields are NoChange`). | Dan/BD | done — moved to ModelOrchestrator/JournalEntryExternalReferenceOrchestration.fs, shaped like the comment no-op test |
...
## Test remediation notes — BD, 2026-08-20

Every assertion written or changed for these items was mutation-tested: the expected value was
