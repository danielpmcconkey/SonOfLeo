...
**Source:** Doctrines.md, Type Validation Doctrine — Validation Layers
...
2. **`validateThenConstruct`** — cross-field constraints within a single record (e.g., type/subtype combinations, activeEnd >= activeBegin).
...
## What works
- VTC validates the record's shape; operation functions validate the world's state; both pass before persistence
- Composite validation ordering follows domain needs, not a rigid sequence

## What doesn't
- Skipping a layer because "we checked it elsewhere"
- Putting state-dependent checks (layer 4) inside VTC (layer 2) — VTC has no access to external state
